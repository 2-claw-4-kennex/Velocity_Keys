...
// Rich sentences - varied punctuation, no accidentals
const SAMPLE_TEXTS = [
  ...
  `Programming isn't just logic; it's art. You write code, debug it, refactor it - then watch it finally work!`,
  ...
  `What separates 80 WPM from 160 WPM? Not raw speed - it's slow transitions: 'qu', 'wh', 'ck', and 'ph' that cost you.`,
];
...
// Returns array of lines, each line = array of {char, globalIdx, x, y, w}
// Words are NEVER split across lines - whole word moves to next line.
// Must be called with a ctx that already has font set.
function wordWrapLayout(text, ctx, availW, padL, padT, lineH) {
  // Measure every char width (monospace is fixed, but punctuation differs in prop fonts)
  // We measure per-char for accuracy.
  const chars = text.split("");
  const charWidths = chars.map(ch => ctx.measureText(ch).width);

  // Split text into tokens: alternating word-tokens and space-tokens
  // Each token = { text, startIdx, chars[] }
  const tokens = [];
  let i = 0;
  while (i < chars.length) {
    if (chars[i] === " ") {
      ...
      tokens.push({ isSpace: true, startIdx: start, endIdx: i });
    } else {
      const start = i;
      while (i < chars.length && chars[i] !== " ") i++;
      tokens.push({ isSpace: false, startIdx: start, endIdx: i });
    }
  }

  // Lay out tokens into lines, never breaking inside a word
  const lines = []; // each line = [{char, globalIdx, x, y, w}]
  let curLine = [];
  let curX = padL;
  let lineIdx = 0;
  const y = () => padT + lineIdx * lineH;

  for (const tok of tokens) {
    if (tok.isSpace) {
      // spaces: add to current line one by one (they follow a word)
      for (let k = tok.startIdx; k < tok.endIdx; k++) {
        curLine.push({ char: chars[k], globalIdx: k, x: curX, y: y(), w: charWidths[k] });
        curX += charWidths[k];
      }
    } else {
      // word: measure total width
      let wordW = 0;
      for (let k = tok.startIdx; k < tok.endIdx; k++) wordW += charWidths[k];

      // If word doesn't fit on current line AND the line already has content → wrap
      if (curX + wordW > padL + availW && curLine.length > 0) {
        // trim trailing spaces from current line before wrapping
        lines.push(curLine);
        lineIdx++;
        curX = padL;
        curLine = [];
...
// Flatten layout to a lookup: globalIdx → {x, y, w}
function buildCharMap(lines) {
  const map = {};
  for (const line of lines)
    for (const c of line)
      map[c.globalIdx] = { x: c.x, y: c.y, w: c.w };
  return map;
}
...
function buildDrillText(slowPairs) {
  ...
  for (const pair of slowPairs.slice(0, 5)) {
    const p = pair.trim().toLowerCase().replace(/[^a-z]/g, "");
    if (!p) continue;
    if (WORD_BANK[p]) for (const w of WORD_BANK[p]) { if (!seen.has(w)) { seen.add(w); pool.push(w); } }
...
function computeMetrics(ksArray, text, duration, targetWpm) {
  ...
  for (let i = 1; i < ksArray.length; i++) {
    ...
    if (lat > 0 && lat < 1500) {
      const fk = prev.k.toLowerCase(), tk = curr.k.toLowerCase();
      ...
      if (/^[a-z ]$/.test(tk)) { if (!keyLats[tk]) keyLats[tk] = []; keyLats[tk].push(lat); }
    }
    if (!curr.ok) { errors++; keyErrors[curr.k] = (keyErrors[curr.k] || 0) + 1; }
  }
...
const FONT_SIZE = 18;
const LINE_H    = Math.round(FONT_SIZE * 2.1);
const PAD_L     = 28;
const PAD_T     = 32;
const PAD_R     = 28;
const MONO_FONT = `${FONT_SIZE}px ui-monospace,"SF Mono","Cascadia Code",Menlo,monospace`;

function getColors() {
  const dark = window.matchMedia("(prefers-color-scheme:dark)").matches;
  return {
    pending:    dark ? "#71717a" : "#9ca3af",
    correct:    dark ? "#22c55e" : "#15803d",
    wrong:      dark ? "#f87171" : "#b91c1c",
    wrongBg:    dark ? "rgba(239,68,68,0.20)" : "rgba(239,68,68,0.12)",
    wordHl:     dark ? "rgba(127,119,221,0.14)" : "rgba(127,119,221,0.10)",
    wordLine:   "#7F77DD",
    cursorClr:  "#7F77DD",
  };
}

const TextDisplay = forwardRef(({ text }, ref) => {
  const canvasRef  = useRef(null);
  const stateRef   = useRef({ typed: "", words: [], curWordIdx: 0 });
  const blinkRef   = useRef(true);
  const textRef    = useRef(text);
  textRef.current  = text;
  // cache layout so blink interval doesn't recompute
  const layoutCache = useRef({ charMap: {}, lineCount: 3, text: "" });
  ...
  function rebuildLayout(ctx, W) {
    const t = textRef.current;
    if (layoutCache.current.text === t && layoutCache.current.W === W) return layoutCache.current;
    const availW = W - PAD_L - PAD_R;
    const lines  = wordWrapLayout(t, ctx, availW, PAD_L, PAD_T, LINE_H);
    const charMap = buildCharMap(lines);
    const cache = { charMap, lines, lineCount: lines.length, text: t, W };
  ...
  function drawFrame() {
    ...
    const { charMap, lines, lineCount } = rebuildLayout(ctx, W);
    const t = textRef.current;

    ctx.clearRect(0, 0, W, (lineCount + 1) * LINE_H + PAD_T * 2);

    // ── Active word highlight underline ──
    if (words.length > 0 && curWordIdx < words.length) {
      const cw = words[curWordIdx];
      const startM = charMap[cw.start];
      const endM   = charMap[cw.end - 1];
      if (startM && endM) {
        const ux = startM.x - 2;
        const uy = startM.y;
        const uw = endM.x + endM.w - startM.x + 4;

        // subtle background box
        ctx.fillStyle = C.wordHl;
        ctx.beginPath();
        ctx.roundRect(ux, uy - FONT_SIZE * 1.3, uw, FONT_SIZE * 1.76, 5);
        ctx.fill();

        // underline bar
        ctx.fillStyle = C.wordLine;
        ctx.fillRect(ux, uy + FONT_SIZE * 0.46, uw, 2.5);
      }
    }

    // ── Draw each character ──
    ctx.font = MONO_FONT; // re-set after fillText sometimes resets in some browsers
    for (let gi = 0; gi < t.length; gi++) {
      ...
      if (isWrong) {
        ...
        ctx.fillRect(m.x, m.y - FONT_SIZE * 1.3, m.w + 1, FONT_SIZE * 1.76);
      }

      // blinking cursor beam (vertical bar before this char)
      if (isCursor && blinkRef.current) {
        ctx.fillStyle = C.cursorClr;
        ctx.fillRect(m.x - 2, m.y - FONT_SIZE * 1.3, 2.5, FONT_SIZE * 1.76);
      }
    ...
    if (typed.length === t.length && blinkRef.current && t.length > 0) {
      ...
      if (last) {
        ctx.fillStyle = C.cursorClr;
        ctx.fillRect(last.x + last.w + 1, last.y - FONT_SIZE * 1.3, 2.5, FONT_SIZE * 1.76);
      }
    }
  }
  ...
  useEffect(() => {
    ...
    if (!cvs) return;
    // Invalidate layout cache on new text
    layoutCache.current = { charMap: {}, lineCount: 3, text: "", W: 0 };
    cvs._update = (typed, words, curWordIdx) => {
      stateRef.current = { typed, words, curWordIdx };
      blinkRef.current = true; // always show cursor immediately after keypress
      drawFrame();
    };
    if (ref) ref.current = cvs;
    // initial draw
  ...
  useEffect(() => {
    const ro = new ResizeObserver(() => {
      layoutCache.current.W = 0; // invalidate width-dependent cache
      drawFrame();
    });
  ...
  // Estimate canvas height: rough line count × LINE_H + padding
  const estLines = useMemo(() => {
    // Very rough: avg 55 chars per line at ~870px wide
    return Math.max(3, Math.ceil(text.length / 52));
  }, [text]);
  const canvasH  = estLines * LINE_H + PAD_T * 2;
  ...
  return (
    <div style={{ position: "relative" }}
      ...
      <div style={{
        ...
        background:bg, color:fg, border:"1px solid rgba(0,0,0,0.10)",
...
function DrillProgress({ baseline, current }) {
  ...
  return (
    <div style={{ padding:"1.25rem 1.5rem", background:"var(--color-background-secondary)", borderRadius:"var(--border-radius-lg)", borderLeft:"3px solid #7F77DD", marginBottom:"1.5rem" }}>
      ...
      {improvements.length === 0 && <p style={{ fontSize:13, color:"var(--color-text-secondary)", margin:0 }}>Not enough digraph overlap yet. Try another session.</p>}
    </div>
  );
}
...
export default function TypeForge() {
  ...
  // Hot-path refs - no re-render on keypress
  ...
  const pushCanvas = useCallback((typed) => {
    ...
    const curWordIdx = getCurrentWordIdx(wordsRef.current, typed.length);
    cvs._update(typed, wordsRef.current, curWordIdx);
  }, []);
  ...
  const startTest = useCallback((opts = {}) => {
    ...
    setTimeout(() => {
      ...
      const txt = (newMode ?? modeRef.current) === "drill"
        ? (drillText ?? drillTextRef.current)
        : SAMPLE_TEXTS[resolvedIdx];
      wordsRef.current = getWords(txt);
      const cvs = canvasRef.current;
  ...
  const handleKeyDown = useCallback(e => {
    // Block all paste / select-all shortcuts
    ...
    if (e.key === "Backspace") {
      ...
      // Only allow deleting back to the start of the current word
      const curWordIdx = getCurrentWordIdx(wordsRef.current, prev.length);
      const wordStart  = wordsRef.current[curWordIdx]?.start ?? 0;
      if (prev.length > wordStart) {
        next = prev.slice(0, -1);
        if (ksRef.current.length > 0) ksRef.current.pop();
      }
      // If already at word start, do nothing - cannot go to previous word
    } else {
      const ch  = e.key;
      const pos = prev.length;
      if (pos >= text.length) return;

      // ── Space gate: only allowed when current word is fully & correctly typed ──
      if (ch === " ") {
        const curWordIdx = getCurrentWordIdx(wordsRef.current, pos);
        const cw = wordsRef.current[curWordIdx];
        if (!cw) return;
        const wordTyped  = prev.slice(cw.start, pos);
      ...
      if (startTimeRef.current) {
        ...
        const wordCt  = spaces + (next[next.length - 1] === " " ? 0 : 1);
        setLiveWpm(Math.min(300, Math.round(wordCt / Math.max(elapsed, 0.0001))));
      }
    }
    ...
    // ── Completion gate: only if typed === text exactly (100% accuracy) ──
  ...
  return (
    <div style={{ fontFamily:"var(--font-sans)", maxWidth:870, margin:"0 auto", padding:"0 0 4rem" }}>
      <h2 className="sr-only">TypeForge</h2>
      ...
      <div style={{ padding:"1.5rem 0 1.25rem", borderBottom:"0.5px solid var(--color-border-tertiary)", marginBottom:"1.75rem", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
          ...
          {mode==="drill" && <button onClick={nextDiagnostic} style={{ fontSize:13, padding:"7px 14px" }}>← Diagnostic</button>}
      ...
      {view==="test" && (
        <div>
          ...
          {/* Invisible input - all logic in onKeyDown */}
          <input
            ref={inputRef}
            value={typedDisplay}
            onChange={()=>{}}
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:8, flexWrap:"wrap", gap:8 }}>
            <p style={{ fontSize:12, color:wordError?"#E24B4A":"var(--color-text-secondary)", margin:0, transition:"color 0.2s", fontWeight:wordError?500:400 }}>
              {wordError
                ? "⚠ Fix the current word completely before pressing Space"
                : "Click the text to focus · Fix every word before advancing · Paste is blocked"}
            </p>
            <div style={{ display:"flex", gap:12, fontSize:12, color:"var(--color-text-secondary)" }}>
              <span style={{ display:"flex", alignItems:"center", gap:4 }}><span style={{ width:9, height:9, borderRadius:"50%", background:"#22c55e", display:"inline-block" }}/>correct</span>
              <span style={{ display:"flex", alignItems:"center", gap:4 }}><span style={{ width:9, height:9, borderRadius:"50%", background:"#f87171", display:"inline-block" }}/>wrong</span>
              <span style={{ display:"flex", alignItems:"center", gap:4 }}><span style={{ width:9, height:9, borderRadius:"50%", background:"#9ca3af", display:"inline-block" }}/>pending</span>
            </div>
          </div>
        </div>
      )}
      ...
      {view==="results" && metrics && (
        <div>
          ...
          <div style={{ display:"flex", gap:2, overflowX:"auto", borderBottom:"0.5px solid var(--color-border-tertiary)", marginBottom:"1.5rem" }}>
            {[{id:"overview",label:"Overview"},{id:"digraphs",label:"Digraph analysis"},{id:"heatmap",label:"Keyboard heatmap"},{id:"fingers",label:"Finger analytics"},{id:"drills",label:"Personalized drills"}]
                ...
                <ol style={{ margin:0, paddingLeft:"1.25rem", fontSize:13, color:"var(--color-text-secondary)", lineHeight:2.2 }}>
                  {metrics.slowDigraphs.slice(0,3).map(d=>(
                    <li key={d.pair}>
                      ...
                      {" "}→ <span style={{ fontFamily:"var(--font-mono)", color:"#E24B4A", fontWeight:600 }}>{d.avg}ms</span> → <span style={{ fontFamily:"var(--font-mono)", color:"#1D9E75", fontWeight:600 }}>{metrics.targetLatency}ms</span>
                    </li>
                  ))}
              ...
              {metrics.sortedDigraphs.slice(0,18).map(([pair,s],i)=>{
                ...
                return (
                  <div key={pair} style={{ display:"flex", alignItems:"center", gap:10, padding:"7px 0", borderBottom:"0.5px solid var(--color-border-tertiary)" }}>
                    ...
                    <span style={{ fontSize:11, color:"var(--color-text-secondary)", minWidth:40 }}>{s.count}×</span>
                  </div>
                );
              })}
            </div>
          ...
          {activeTab==="heatmap" && (
            <div>
              ...
              <p style={{ fontSize:13, color:"var(--color-text-secondary)", marginBottom:"1.25rem" }}>Hover any key for full stats. Target: <b>{metrics.targetLatency}ms</b>.</p>
              <div style={{ display:"flex", flexDirection:"column", gap:6, alignItems:"flex-start" }}>
                ...
                <div style={{ paddingLeft:148 }}>
                  <div style={{ width:234, height:54, display:"flex", alignItems:"center", justifyContent:"center", borderRadius:8, border:"1px solid rgba(0,0,0,0.10)", fontWeight:500, fontSize:13, background:metrics.keyHeatmap[" "]?"#1D9E75":"var(--color-background-secondary)", color:metrics.keyHeatmap[" "]?"#fff":"var(--color-text-secondary)" }}>
          ...
          {activeTab==="drills" && (
            <div>
              <h3 style={{ fontSize:16, fontWeight:500, margin:"0 0 0.5rem" }}>{mode==="drill"?"Run another drill":"Personalized drill"}</h3>
              <p style={{ fontSize:13, color:"var(--color-text-secondary)", margin:"0 0 1.25rem", lineHeight:1.6 }}>
                {mode==="drill"
                  ? "Drill complete. Run again to consolidate muscle memory, or return to diagnostic."
              ...
              <div style={{ display:"flex", gap:10 }}>
                <button onClick={()=>startDrillSession(metrics)} style={{ fontSize:14, padding:"11px 28px", background:"#7F77DD", color:"#fff", border:"none", borderRadius:"var(--border-radius-md)", cursor:"pointer", fontWeight:500 }}>
                  Start drill session →
                </button>
                {mode==="drill" && <button onClick={nextDiagnostic} style={{ fontSize:14, padding:"11px 20px" }}>Back to diagnostic</button>}
              </div>
            </div>
          )}
        </div>
